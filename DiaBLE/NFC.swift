...
class NFC: NSObject, NFCTagReaderSessionDelegate, Logging {
    ...
    public func tagReaderSession(_ session: NFCTagReaderSession, didDetect tags: [NFCTag]) {
        ...
        session.connect(to: firstTag) { [self] error in
            if error != nil {
            ...
            connectedTag?.getSystemInfo(requestFlags: .highDataRate) { result in
                ...
                connectedTag?.customCommand(requestFlags: .highDataRate, customCommandCode: 0xA1, customRequestParameters: Data()) { result in
            ...
            case .failure(let error):
                ...
                if retries > 0 {
                    ...
                    if sensor.securityGeneration < 2 || taskRequest == .none {
                        tagSession?.invalidate(errorMessage: "Error while reading multiple blocks: \(error.localizedDescription.localizedLowercase)")
                    }
