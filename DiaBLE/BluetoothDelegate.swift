...
class BluetoothDelegate: NSObject, CBCentralManagerDelegate, CBPeripheralDelegate, Logging {
    ...
    public func centralManager(_ manager: CBCentralManager, didDiscover peripheral: CBPeripheral, advertisementData advertisement: [String : Any], rssi: NSNumber) {
        ...
        if advertisement[CBAdvertisementDataIsConnectable] as? Int == 0
            ...
            var msg = "Bluetooth: skipped \(name!)"
    ...
    public func peripheral(_ peripheral: CBPeripheral, didDiscoverCharacteristicsFor service: CBService, error: Error?) {
        ...
        for characteristic in characteristics {
            ...
            if uuid == Abbott.dataReadCharacteristicUUID || uuid == Bubble.dataReadCharacteristicUUID || uuid == MiaoMiao.dataReadCharacteristicUUID {
        ...
        if app.device.type == .transmitter(.abbott) && serviceUUID == Abbott.dataServiceUUID {
            ...
            if settings.activeSensorSerial == app.device.serial {
                sensor.activePatchInfo = settings.activeSensorPatchInfo
                ...
                log("Bluetooth: the active sensor \(app.device.serial) has reconnected: restoring settings: active patch info: \(sensor.activePatchInfo.hex), current patch info: \(sensor.patchInfo.hex), unlock count: \(sensor.unlockCount)")
            }
            ...
            } else if sensor.uid.count > 0 && sensor.patchInfo.count > 0 && settings.activeSensorPatchInfo.count > 0 {
                sensor.unlockCount += 1
                settings.activeSensorUnlockCount += 1
                let unlockPayload = Libre2.streamingUnlockPayload(id: sensor.uid, info: settings.activeSensorPatchInfo, enableTime: sensor.unlockCode, unlockCount: sensor.unlockCount)
                log("Bluetooth: writing streaming unlock payload: \(Data(unlockPayload).hex) (patch info: \(settings.activeSensorPatchInfo.hex), unlock code: \(sensor.unlockCode), unlock count: \(sensor.unlockCount), sensor id: \(sensor.uid.hex), current patch info: \(sensor.patchInfo.hex))")
